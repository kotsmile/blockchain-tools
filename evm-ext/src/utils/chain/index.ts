...
export const node = (name: ChainTag): Node => {
  if (name === 'localhost') {
    return {
...
export const getConfig = (name: ChainTag): Config => {
  if (Object.keys(names).includes(name)) {
    return {
      name: names[name],
      chainId: chainIds[name],
...
export const getChainTag = (chainId: ChainId) => chainTags[chainId]
export const getChainName = (chainId: ChainId) => node(getChainTag(chainId)).name
export const getChainHex = (chainId: ChainId) =>
  '0x' + node(getChainTag(chainId)).chainId.toString(16)
export const getChainScanner = (chainId: ChainId) =>
  '0x' + node(getChainTag(chainId)).scanner
export const getChainDescription = (chainId: ChainId) => getConfig(getChainTag(chainId))
